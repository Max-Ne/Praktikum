...
for i in range(len(filenames)):
  ...
  for line in dataf:
    ...
    x[i].append(np.log(float(line[0][:-2].replace(',','.'))))
    y[i].append(np.log(float(line[1][:-2].replace(',','.'))))
...
mg.SetTitle("Photostrom #ddot{u}ber Beleuchtungsst#ddot{a}rke;log (cos^{2}(#Theta));log (I / mA)")
